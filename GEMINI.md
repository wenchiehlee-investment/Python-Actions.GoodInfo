...
This repository contains a set of Python scripts designed to automatically download 12 different types of stock data from the Taiwanese stock information website, `GoodInfo.tw`.
...
*   **`Get觀察名單.py`**: A utility script to download the master list of stock IDs (`StockID_TWSE_TPEX.csv`) from an external GitHub source.
*   **Data Directories**: Folders like `DividendDetail/`, `StockBzPerformance/`, etc., where the downloaded `.xls` files and `download_results.csv` logs are stored.
*   **GitHub Actions Workflow (`sync.yml`):** The automation scheduler that runs `GetAll.py` for different data types on a predefined weekly, daily, and monthly schedule.
...
The script takes a stock ID and a numeric data type code (1-12).
...
*   **Robustness**: The downloader (`GetGoodInfo.py`) has multiple fallback methods to find the download button, and the batch processor (`GetAll.py`) has a multi-attempt retry mechanism with cleanup actions between tries.

## 5. Automation

The repository uses a GitHub Actions workflow to run `GetAll.py` on a schedule. The schedule is designed to distribute the load on the `GoodInfo.tw` server by downloading different data types on different days and times.

*   **File**: The workflow is defined in `.github/workflows/sync.yml`.
*   **Triggers**: The workflow runs on a `schedule` (cron) and can also be triggered manually (`workflow_dispatch`).
*   **Logic**: The workflow script determines which `DATA_TYPE` to run based on the day and time.
