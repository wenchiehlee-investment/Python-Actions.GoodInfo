...
This repository contains a set of Python scripts designed to automatically download 18 different types of stock data from the Taiwanese stock information website, `GoodInfo.tw`.
...
*   **Data Directories**: Folders like `DividendDetail/`, `ShowMarginChart/`, `ShowWeeklyK_ChartFlow/`, `ShowDailyK_ChartFlow/`, etc., where the downloaded 18 categories of `.xls` files and `download_results.csv` logs are stored.
...
The repository uses GitHub Actions workflows to run `GetAll.py` on a schedule. The schedule is designed to distribute the load on the `GoodInfo.tw` server by downloading 18 different data types on predefined weekly, daily, and monthly schedules.
